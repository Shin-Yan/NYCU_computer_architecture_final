...
bool matrixEqual(Matrix* m1, Matrix* m2){
    ...
    for(int i = 0 ; i < m1->rows_ ;++i){
        for(int j = 0 ; j < m1->cols_ ; ++j){
            if(m1->mat[i*m1->cols_+j] != m2->mat[i*m2->cols_+j] ){
...
void dumpIndex(IndexSave* Ind, Matrix* cpu_mat, Matrix* gpu_mat){
    for(int i = 0 ; i < gpu_mat->rows_ ; ++i ){
        printf("%d : blockInd_x=%d, threadInd_x=%d, head=%d, stripe=%d\n", i, Ind[i].blockInd_x, Ind[i].threadInd_x, Ind[i].head, Ind[i].stripe);
        printf("    GPU result :");
        for(int j = 0 ; j < gpu_mat->cols_ ; ++j){
            printf(expr_m, gpu_mat->mat[i*gpu_mat->cols_ + j]);
        }
        printf("\n");
        printf("    CPU result :");
        for(int j = 0 ; j < cpu_mat->cols_ ; ++j){
            printf(expr_m, cpu_mat->mat[i*cpu_mat->cols_ + j]);
        }
        printf("\n");
    }
}
